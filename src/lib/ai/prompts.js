export function createRagSystemPrompt({
  ...
  const rules = [
    ...
    "不要使用 [2-4]、[2,3] 这类范围引用或合并引用；需要多个来源时分别写成 [2][3][4]。",
    "总结、概括、提炼类回答要控制引用密度：每个句子或列表项最多放 1-2 个最能支持该点的引用，不要为了覆盖所有资料而在同一句或同一段堆叠大量引用。",
    "如果某个分类包含很多知识点，应拆成多个子要点分别说明，而不是把大量知识点和大量引用挤在同一个列表项里。",
    "尤其是面经总结中，不要把所有八股题或知识点写成一个超长列表项；应按 CSS、JavaScript、React、程序输出题、手撕代码等拆分成多个短要点。",
